...
export default function WoMensCollections() {
  ...
  // Filter Logic: Includes products that are explicitly "WoMen" or "Both" (Unisex)
  const womensProducts = products.filter((p) =>
    p.category.some(
      (cat) =>
        cat.toLowerCase() === "women" ||
        cat.toLowerCase() === "womens" ||
        cat.toLowerCase() === "both" ||
        cat.toLowerCase() === "unisex"
    )
  );
  useEffect(() => {
        ...
        <div className="px-4 md:px-6 py-6 max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-gray-400 mt-2 uppercase text-[9px] md:text-[10px] tracking-[0.3em] font-bold">
              High-performance essentials for him
            </p>
          </div>
          <div className="text-[10px] font-bold uppercase tracking-widest text-gray-400">
            {womensProducts.length} Products Found
          </div>
        </div>
      </header>
                ...
                <div className="mt-auto">
                  ...
                  <div className="pt-4 border-t border-gray-100 flex flex-col gap-4">
                    ...
                    <a
                      ...
                      className="w-full hover:text-black hover:bg-white bg-black text-center text-white py-3 md:py-4 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] transition-all duration-300 hover:bg-gray-800 active:scale-[0.98] shadow-lg shadow-black/5"
                    >
                      Buy Now
                    </a>
                  </div>
